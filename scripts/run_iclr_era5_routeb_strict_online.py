...
def main():
    ...
    if args.representation == "analytic_hippo_rff":
        ...
        if solver_backend == "torch":
            ...
            z_t = np.linspace(float(times.min()), float(times.max()), args.mt)
            ...
            kfu = matern32_1d(
                times, z_t, float(theta["ell_t"]), float(theta["kernel_variance"])
            )
    ...
    all_mean = []
    all_var = []
    prediction_grid = np.empty((times.size, test_indices.size), dtype=np.float64)
    ...
    final_block = {key: rows[-1][key] for key in ("rmse", "nll", "coverage90", "mean_predictive_std")}
    persistent_bytes = rows[-1]["persistent_state_bytes"]
    payload = {
        "implementation": "Route B strict-online structured joint beta-GP",
        "protocol": "Task-1 Route-B empirical-Bayes calibration; frozen theta; new-block-only Task-2(+) streaming",
