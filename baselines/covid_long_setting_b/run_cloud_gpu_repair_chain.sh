...
WAIT_PID_FILE="${WAIT_PID_FILE:-$ROOT/baselines/covid_long_setting_b/results/convergence_repair_v1/gpu_execution_plan/factorial_lmc_imc_capacity_v2.pid}"
...
FACTORIAL_ROOT="${FACTORIAL_ROOT:-$ROOT/baselines/covid_long_setting_b/results/convergence_repair_v1/gpu_execution_plan/blocked_development_factorial_v2}"
...
' "$OHSVGP_GATE_ROOT/gate_status.json"; then
    "$PYTHON" baselines/covid_long_setting_b/run_blocked_development.py \
        ...
        --methods ohsvgp --phase capacity --execute --ohsvgp-device cuda
else
