...
\begin{equation}
    ...
    a_*^\top:=K_{*u}K_{uu}^{-1}.
\end{equation}
Here \(K_{uu}\) and \(K_{*u}\) are evaluated in the current inducing basis, and
\(a_*\) denotes the corresponding column vector.  Let the structured joint
precision be partitioned as in \eqref{eq:schur_block_notation}, with
\(D_u=\Lambda_{uu}\) and \(B_{\beta u}=\Lambda_{\beta u}\).  The predictive mean is
\begin{equation}
    \E[y_*]
    =
...
The sparse GP conditional residual variance is
\begin{equation}
    \nu_*
    :=
    k(x_*,x_*)-K_{*u}K_{uu}^{-1}K_{u*}.
    \label{eq:sparse_conditional_residual_variance}
\end{equation}
This is the conditional variance of \(f_*\) given the inducing variables \(u\).
...
\end{equation}
This quantity can be evaluated without materializing the full covariance
\(S=\Lambda^{-1}\).  First solve
\begin{equation}
    D_uv_*=a_* .
    \label{eq:prediction_du_solve}
\end{equation}
...
\[
    ...
    A_\beta-B_{\beta u}D_u^{-1}B_{\beta u}^\top
\]
from \eqref{eq:beta_schur_complement}, we have
\begin{equation}
    \begin{bmatrix}\phi_*\\a_*\end{bmatrix}^{\!\top}
    \Lambda^{-1}
    \begin{bmatrix}\phi_*\\a_*\end{bmatrix}
...
\end{equation}
Therefore the predictive variance for the noisy observation is
\begin{equation}
    \Var(y_*)
    =
    \sigma^2+
...
posterior \(\beta\)--\(u\) cross covariance implicitly.  If the full covariance
\(S=\Lambda^{-1}\) were materialized and partitioned as
\[
    S=
    \begin{bmatrix}
    ...
    \end{bmatrix},
\]
then the posterior-parameter uncertainty would expand as
\begin{equation}
    \phi_*^\top S_{\beta\beta}\phi_*
    +a_*^\top S_{uu}a_*
    +2\phi_*^\top S_{\beta u}a_* .
    \label{eq:full_covariance_predictive_expansion}
\end{equation}
The cross term \(2\phi_*^\top S_{\beta u}a_*\) is absent under a mean-field
posterior approximation.  Route B keeps this contribution through the
Schur-complement formula above, while requiring only structured solves with the
\(u\)-block and low-dimensional solves with \(\Lambda_{\beta\mid u}\).
...
\end{equation}
which drops the posterior cross-covariance contribution and should therefore be
treated as a calibration ablation rather than the main predictive distribution.

\paragraph{Kronecker/Sylvester evaluation of the prediction terms.}
In the spatio-temporal Kronecker implementation, the inducing variable is
arranged according to the same vectorization convention as in
Section~\ref{subsec:sylvester_reformulation}:
\[
    u=\vecop(U),
    \qquad
    U\in\mathbb{R}^{M_s\times M_t}.
\]
Thus the \(u\)-block precision has the Sylvester-compatible form
\[
    D_u
    =
    (K_{nn}^{(t)})^{-1}\otimes K_s^{-1}
    ...
    B_n\otimes G,
\]
where \(K_s=K_{ZZ}^{(s)}\) and \(G=C^\top C\).

For a test point \(x_*=(t_*,s_*)\), define the one-point temporal and spatial
projection rows by
\begin{equation}
    a_{t,*}^{\top}
    :=
    k_{fu}^{(t)}(t_*;T_n)\left(K_{nn}^{(t)}\right)^{-1},
    \qquad
    a_{s,*}^{\top}
    :=
    k_s(s_*,Z_s)K_s^{-1}.
\end{equation}
This is the pointwise analogue of \(A_n=T_n\otimes C\) in
\eqref{eq:A-kron}.  With the vectorization convention above,
\begin{equation}
    a_*^\top
    =
    a_{t,*}^{\top}\otimes a_{s,*}^{\top},
    \qquad
    a_*
    =
    a_{t,*}\otimes a_{s,*}
    =
    \vecop\!\left(a_{s,*}a_{t,*}^{\top}\right).
    \label{eq:prediction_projection_kron}
\end{equation}
This ordering is important: it is consistent with the identity
\[
    (A\otimes B)\vecop(Z)=\vecop(BZA^\top)
\]
used in the Sylvester formulation.

The solve \(D_uv_*=a_*\) is therefore not implemented by forming the dense
matrix \(D_u\in\mathbb{R}^{M_tM_s\times M_tM_s}\).  Writing
\[
    v_*=\vecop(V_*),
    \qquad
    V_*\in\mathbb{R}^{M_s\times M_t},
\]
and
\[
    a_*=\vecop(A_*),
    ...
    A_*:=a_{s,*}a_{t,*}^{\top},
\]
the structured \(u\)-block solve is evaluated through the Sylvester-type equation
\begin{equation}
    K_s^{-1}V_*(K_{nn}^{(t)})^{-1}
    +
    GV_*B_n
    =
    A_* .
    \label{eq:prediction_sylvester_solve}
\end{equation}
This is the same Kronecker/Sylvester representation used in posterior recovery.
It avoids materializing either \(D_u^{-1}\) or the full posterior covariance
\(\Lambda^{-1}\).

For a batch of test points, the same marginal variance formula is evaluated in
chunks.  For each test point \(x_{*,i}\), solve
\[
    D_uv_{*,i}=a_{*,i},
\]
and define
...
\begin{equation}
    ...
    \Lambda_{\beta\mid u}^{-1}
    \tilde{\phi}_{*,i}.
    \label{eq:chunked_predictive_variance}
\end{equation}
Chunking is a numerical implementation detail only; it does not change the
marginal predictive variance.  Equation~\eqref{eq:chunked_predictive_variance}
computes pointwise marginal predictive variances.  If a joint Gaussian
predictive distribution over a whole test batch is required, the off-diagonal
predictive covariances between test points must also be computed.

\paragraph{Non-unit kernel amplitude in the sparse residual variance.}
The sparse conditional residual variance must use the same prior point variance
as the GP generative model.  Thus,
\begin{equation}
    \nu_*
    =
    k(x_*,x_*)
    ...
    K_{*u}K_{uu}^{-1}K_{u*},
\end{equation}
where \(k(x_*,x_*)\) includes the kernel amplitude whenever an amplitude
parameter is present.  For example, if the separable kernel is written as
\[
    k((t,s),(t',s'))
    =
    \alpha^2 k_t(t,t')k_s(s,s'),
\]
then
\begin{equation}
    k(x_*,x_*)
    =
    \alpha^2 k_t(t_*,t_*)k_s(s_*,s_*).
\end{equation}
For normalized temporal and spatial correlation kernels,
\(k_t(t_*,t_*)=k_s(s_*,s_*)=1\), so
\[
    k(x_*,x_*)=\alpha^2,
\]
not necessarily one.  Although the amplitude may cancel in the projection
\(K_{*u}K_{uu}^{-1}\) when it is included consistently in both \(K_{*u}\) and
\(K_{uu}\), it does not disappear from the residual variance \(\nu_*\).  This
distinction is important for negative log likelihood and coverage: if the data
were generated or fitted with a non-unit kernel amplitude, replacing
\(k(x_*,x_*)\) by one would mis-scale the predictive variance.

\paragraph{Latent-function versus noisy-observation uncertainty.}
Equation~\eqref{eq:structured_predictive_variance} is the predictive variance
for the noisy observation
\[
    y_*=\phi_*^\top\beta+f_*+\epsilon_*,
    \qquad
    \epsilon_*\sim\mathcal{N}(0,\sigma^2).
\]
The corresponding latent-function variance, excluding observation noise, is
\begin{equation}
    \Var(f_*+\phi_*^\top\beta)
    =
    \nu_*+
    ...
    \Lambda_{\beta\mid u}^{-1}
    \left(\phi_*-B_{\beta u}v_*\right).
    \label{eq:latent_predictive_variance}
\end{equation}
NLL and coverage for observed test labels must use the noisy-observation
variance, including \(\sigma^2\).  If coverage is evaluated against the latent
function instead, the \(\sigma^2\) term should be omitted.  Throughout the
experiments, the reported NLL and coverage are computed for noisy observations,
so they use the variance in
\eqref{eq:structured_predictive_variance}.
